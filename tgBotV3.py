...
logger = logging.getLogger('tgBotV3') # Changed from tgBotV2 to tgBotV3
...
def extract_trade_info(message):
    ...
    close_keywords = ['空止盈', '空止损', '多止盈', '多止损', '平多', '平空', 'MA止损']
...
def extract_close_signal(message):
    ...
    close_type = None
    if any(kw in message for kw in ['执行交易:MA止损', 'MA止损']):
        close_type = 'both'
...
def build_close_bark_content(close_type, symbol, account_name, close_results, okx_resp=None, error_msg=None):
    now = get_shanghai_time()
    close_type_display = "多空双向" if close_type == 'both' else close_type
    lines = [f"账户: {account_name}", f"交易标的: {symbol}", f"信号类型: 平仓{close_type_display}", f"平仓结果: {len(close_results)} 个持仓", f"时间: {now}"]
...
async def close_okx_position(account, symbol, close_type):
    try:
        ...
        positions_to_close = []
        if close_type == 'both':
            positions_to_close = resp.get('data', [])
        else:
            positions_to_close = [p for p in resp.get('data', []) if p.get('posSide') == close_type]

        for pos in positions_to_close:
            if float(pos.get('pos', '0')) > 0:
                pos_side_to_close = pos.get('posSide')
                side = 'sell' if pos_side_to_close == 'long' else 'buy'
                close_resp = trade_api.place_order(instId=inst_id, tdMode='cross', side=side, posSide=pos_side_to_close, ordType='market', sz=pos['pos'])
